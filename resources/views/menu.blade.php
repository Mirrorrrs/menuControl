            ...
            <div class="card-body">
                ...
                <form class=" day_form w-100" action="{{route("save-menu")}}" method="post">
                    ...
                    <div class="continer-fluid form_{{$day->id}}">
                        @foreach($day->meals as $meal)
                            <div class="form_group">
                                <select name="types[{{$meal->id}}]" class="mt-2 form-control select_type col-3" required>
                    ...
                    <div class="form_group d-flex justify-content-between align-items-center mt-2">
                        ...
                        <a href="javascript:void(0)" class="add_meal btn btn-success" data-target = "form_{{$day->id}}">Добавить</a>
                    </div>
...
Array.from(meal_btns).forEach(el=>{
    el.onclick = ()=>{
        ...
        let proto = new DOMParser().parseFromString(" <div class=\"form_group\">\n" +
            "                      <select name=\"types[new_"+new_count+"]\" class=\"mt-2 form-control col-3\" required>\n" +
